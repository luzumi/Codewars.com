...
public class DirReduction {
        ...
        do {
            if (resultList.size() >= 2) {
                for (int i = 0; i < resultList.size()-1; i++) {
                    if (resultList.get(i) == switchDirection(resultList.get(i + 1))) {
                        check = true;
                        resultList.remove(i);
                        resultList.remove(i);
                        break;
    ...
    public static String switchDirection(String switchedDir) {
        switch (switchedDir) {
            ...
            default : throw new IllegalStateException("Unexpected value: " + switchedDir);
        }
    }
}
