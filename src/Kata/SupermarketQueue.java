...
public class SupermarketQueue {
    public static int solveSuperMarketQueue(int[] customers, int n) {
        ...
        int[] lastCustomerTime = new int[n];
        int counter = 0;
        int customerCount = 0;

        do {
            int actualSlot = counter%n;
            if (lastCustomerTime[actualSlot] == 0) {
                queueList[actualSlot] += customers[customerCount];
                lastCustomerTime[actualSlot] = customers[customerCount];
                counter++;
                customerCount++;
            }
            else {
                lastCustomerTime[actualSlot]--;
                counter++;
            }
        } while (customerCount != customers.length);
        System.out.println(Arrays.toString(queueList));
        Arrays.sort(queueList);
